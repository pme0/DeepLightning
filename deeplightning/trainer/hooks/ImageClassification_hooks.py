...
def training_step__ImageClassification(self, batch, batch_idx):
    ...
    # metrics
    self.metrics["Accuracy_train"].update(preds=outputs, target=batch["targets"])
...
def validation_step__ImageClassification(self, batch, batch_idx):
    ...
    # metrics
    self.metrics["Accuracy_val"].update(preds = preds, target = batch["targets"])
    self.metrics["ConfusionMatrix_val"].update(preds = preds, target = batch["targets"])
    self.metrics["PrecisionRecallCurve_val"].update(preds = outputs, target = batch["targets"])
...
def test_step__ImageClassification(self, batch, batch_idx):
        ...
        self.metrics["ConfusionMatrix_test"].update(preds = preds, target = batch["targets"])
        self.metrics["PrecisionRecallCurve_test"].update(preds = outputs, target = batch["targets"])
